       ...
       "columnDefs":
          ...
          columns: [
            { data: 'status', name: 'status' ,
              ...
              {
                   "render": function (data, type, row, meta){ 
                    // return ('<div class="btn-group"><a href="/autocar/'+row.id+'" class="btn btn-icon icon-left btn-primary"><i class="fas fa-eye"></i>Voir</a><button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span></button><form method="post" action="/autocars/destroy/'+row.id+'">{{ method_field('DELETE') }}<input type="hidden" name="_token" value="{{ csrf_token() }}"><div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; top: 0px; left: 0px; will-change: transform; padding: 0px; width:0;"><button class="dropdown-item btn btn-icon icon-left btn-danger delete-row" style="width:100px;position: inherit;left: 51px;" type="submit" value="'+row.id+'"><i class="far fa-trash-alt"></i> Supprimer</button></div></form></div>');
                    return ('<form method="post" action="/autocars/destroy/' + row.id + '">{{ method_field('DELETE ') }}<input type="hidden" name="_token" value="{{ csrf_token() }}"><button class="btn btn-outline-danger delete-row" type="submit" value="' + row.id + '">Supprimer</button></form>');
